...
export default class PostGallery extends React.Component {
  ...
  static getSize() {
    ...
    return PostGallery.sizes.filter(
      size => (!size.min || size.min <= width) && (!size.max || size.max > width)
    )[0];
  }
  static propTypes = {
  ...
  static sizes = [{
    ...
    max: 0,
    gap: 1,
    // ideally, the row height would differ here too but
    // older devices suck at scaling background images...
    rowHeight: () => 350,
    rowWidth: 1200
  }]
  static getAvailableWidth = () => Utils.getDimensions().width * 0.9;
  constructor(props) {
    super(props);
    this.state = {
      isFunky: false,
  ...
  componentWillReceiveProps(next) {
    ...
    if (!this.getPictures() || !_.isEqual(this.getPictures(), this.getPictures(next))) {
      this.repaint(PostGallery.getSize(), next);
    }
  }
  componentDidUpdate() {
  ...
  onResize() {
    ...
    // repaint when resizing and viewport width lower than 768
    // (never happens in practice, I guess, but hey, I like stuff clean)
    this.width = Utils.getDimensions().width;
    const current = PostGallery.getSize();
  ...
  getPlain() {
    return this.state.pictures.map(picture =>
      <a key={picture.id} className={classnames('plain-image')} onClick={() => this.handlePicutreClick(picture)}>
  ...
  getFunky() {
    return this.state.pictures.map(picture =>
      <a
        key={picture.id}
        className={classnames('funky-image', { last: picture.funky.isLast })}
        onClick={() => this.handlePicutreClick(picture)}
        style={{
  ...
  repaint(size = PostGallery.getSize(), props = this.props) {
    this.size = size;
    if (this.size) {
      this.setState({
        isFunky: true,

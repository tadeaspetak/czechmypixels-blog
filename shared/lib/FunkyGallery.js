...
const FunkyGallery = {
  defaults: {
    ...
    getResizedWidth: (settings, element, rh) =>
      rh * (settings.getWidth(element) / settings.getHeight(element)), // calculate width given row height
    getPotential: (settings, element, rw) =>
      (settings.getWidth(element) > settings.getHeight(element) ? rw * 0.4 : rw * 0.2), //get potential for cropping given element width
    // set the funky properties
    setFunky: (e, cropWidth, height, resizedWidth) => {
      const element = e;
      element.funky = {
        ...
        // (useful e.g. when setting "background-position-x", if not using
        // background-position:center for some reason)
  ...
  arrange(elements, settings, rows) {
    let {row, widths, widthSum, potentials, potentialSum, rowHeight, rowWidth} =
      this.pickRow(elements, settings);
    let gaps = (_.size(row) - 1) * settings.gap;
    let difference = widthSum - rowWidth + gaps;

    // a complete row
    if (difference > 0) {
      // all elements but last -> keep "cropping" the width by a random
      // number using the element's potential as the crop maximum
      _.dropRight(row, 1).forEach((element, index) => {
        potentialSum -= potentials[index];
        const decrease = _.random(
          Math.max(0, difference - potentialSum),
          Math.min(potentials[index], difference)
        );
    ...
    // an incomplete row
      ...
      row.forEach((element, index) => settings.setFunky(
        element, widths[index], rowHeight, widths[index]
      ));
    }
    rows.push(row);

    // ontinue recursively as long as there are elements to be processed
    const remaining = _.drop(elements, _.size(row));
